...
QVector3D GlobeRenderer::Camera::ComputeRayFromScreen(const QPointF &Point) const
{
...
QVector2D GlobeRenderer::Camera::ProjectWorldToScreenSpace(const QVector3D &Pos3D) const
{
...
QVector3D GlobeRenderer::Camera::ProjectScreenToWorldSpace(const QPointF &Point, float Depth) const
{
    // Convert screen position to Normalized Device Coordinates (NDC)
    float NdcX = (2.0f * Point.x()) / mWidth - 1.0f;
    float NdcY = 1.0f - (2.0f * Point.y()) / mHeight;
...
float GlobeRenderer::Camera::GetDepthOfPoint(const QVector3D &Point) const
{
    QMatrix4x4 ViewProjectionMatrix = GetViewProjectionMatrix();
...
QMatrix4x4 GlobeRenderer::Camera::GetViewProjectionMatrix() const
{
    return GetProjectionMatrix() * GetViewMatrix();
}

QMatrix4x4 GlobeRenderer::Camera::GetViewMatrix() const
{
...
QMatrix4x4 GlobeRenderer::Camera::GetRotationMatrix() const
{
...
QVector3D GlobeRenderer::Camera::GetViewDirection() const
{
    return GetRotation() * NEGATIVE_Z; // The view direction is the negative Z axis in the camera's local space
}

QVector3D GlobeRenderer::Camera::GetDirection(ViewDirection ViewDirection) const
{
...
int GlobeRenderer::Camera::GetWidth() const
{
    return mWidth;
}

int GlobeRenderer::Camera::GetHeight() const
{
    return mHeight;
}

QVector2D GlobeRenderer::Camera::GetViewportSize() const
{
